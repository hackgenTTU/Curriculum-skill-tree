...
var classRect = (function() {
    var classRect = function(x, y, title, canvas, name) {
        if (x != null && y != null) {
            this.site = {
                'x': x,
                'y': y
    ...
    var skillTree = function(sourec, canvas) {
        ...
        for (var x in json) {
            ...
            for (var i = 0, x = 20; i < this.classArray.length; i++, x += 70) {
                ...
                this.addNode(x, y, this.classNumArray[i], canvas, this.classArray[i]);
            }
            y += 100;
        }
    }
    skillTree.prototype.addNode = function(x, y, title, canvas, name) {
        var newNode = new classRect(x, y, title, canvas, name);
